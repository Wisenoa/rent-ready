...
Le contrat de location de parking doit mentionner :

- L'identité complète du bailleur et du locataire
- La description précise de l'emplacement (adresse, niveau, numéro de place)
- Le montant du loyer et les modalités de paiement
- La durée du bail et les conditions de renouvellement
- Les charges comprises ou non comprises dans le loyer
- Les conditions de résiliation et le préavis applicable
- Les équipements disponibles (porte automatique, éclairage, gardiennage)
...
- **Fixé** : un montant constant pendant toute la durée du bail
- **Progressif** : augmenté selon un calendrier prédéfini dans le bail
- **Indexé** : lié à un indice (IRL, indice du coût de la construction) pour protéger le propriétaire contre l'inflation
...
- Gros murs et structure du bâtiment
- Ravalement et façades (sauf si liés à l'activité du locataire)
- Mise aux normes d'accessibilité des parties communes
- Installations collectives (ascenseur, chauffage collectif)

**Travaux à la charge du locataire professionnel** :

- Aménagements intérieurs et décoration
- Menuiseries intérieures et équipements propres à l'activité
...
Non, le bail professionnel n'est pas soumis à l'encadrement des loyers applicable aux locations d'habitation. Le loyer est librement fixé entre les parties. Toutefois, certaines zones font l'objet d'un encadrement spécifique pour les renouvellements de bail professionnel.`,
  },
  {
    ...
    title: 'Dépôt de garantie pour location meublée : plafond, restitution et droits en 2026',
...
Ce plafond s'applique à tous les types de location meublée :

- Bail meublé classique (1 an renouvelable)
- Bail mobilité (1 à 10 mois)
- Location meublée de tourisme (saisonnière, avec règles différentes)
...
Le taux légal est actualisé chaque semestre par l'administration. Le bailleur doit fournir un reçu détaillé mentionnant le montant du dépôt initial, les déductions opérées (loyers impayés, charges, dégradations) et le solde restitué.

### Les déductions autorisées sur le dépôt de garantie

Le bailleur peut retenir des sommes sur le dépôt de garantie uniquement pour :

1. **L'impayé de loyer** : si le locataire quitte avec des arriérés de loyer, le bailleur déduit ces sommes du dépôt
2. **Les charges locatives non réglées** : si des provisions de charges n'ont pas été régularisées, le bailleur peut retenir le montant dû
3. **Les dégradations constatées à l'état des lieux de sortie** : autres que l'usure normale, les dégradations imputables au locataire sont déduites du dépôt
...
Le juge examine les pièces présentées par les deux parties : le bail de location, l'état des lieux d'entrée et de sortie, les photos de l'état des lieux, les factures de travaux ou d'achat pour prouver l'état antérieur, les reçus de paiement des charges, et la correspondance entre les parties.

Le juge peut ordonner la restitution partielle ou totale du dépôt, avec intérêts de retard au taux légal. En cas de mauvaise foi manifeste du bailleur (retenue sans motif, absence d'état des lieux), des dommages et intérêts peuvent être octroyés.
...
Le coût de l'expertise est généralement avancé par la partie qui la demande (locataire ou bailleur), et peut être mis à la charge de la partie perdante à l'issue du procès.
...
En cas de défaut de paiement du locataire, le propriétaire peut saisir le juge pour obtenir une condamnation au paiement.
...
Pour le propriétaire : rédigez un état des lieux d'entrée le plus détaillé possible, envoyez le dépôt de garantie dans les 2 mois avec un reçu détaillé même en cas de retenue, conservez les factures des travaux pour prouver les coûts de remise en état, et sottoscrir une GLI pour couvrir les risques.
...
Envoie la lettre tôt est essentiel. Plus le propriétaire attend, plus les arriérés s'accumulent et plus la procédure devient complexe.
...
Il n'y a pas de nombre minimum légal. Une seule lettre de relance peut être suivie d'une mise en demeure. En pratique, un propriétaire envoie généralement une première lettre simple (email ou courrier), puis la lettre recommandée avec accusé de réception, avant la mise en demeure.
...
Si le locataire conteste le montant, vérifiez vos calculs et envoyez un décompte détaillé. Si le litige persiste, saisissez le juge des contentieux de la protection qui tranchera le différend.`,
  },
  {
...
Elle constitue une preuve : la mise en demeure prouve que le propriétaire a formellement alerté le locataire avant d'engager les poursuites. Cette formalité est appréciée par le juge.

Elle ouvre le droit aux intérêts de retard : les intérêts de retard sur les arriérés courent à compter de la mise en demeure, au taux légal.
...
Nous vous rappelons que les sommes dues produisent intérêts au taux légal à compter de la présente mise en demeure.
...
Le locataire peut contester la saisie sur salaire dans plusieurs cas : saisie excédant les limites légales (si la part saisie dépasse les plafonds autorisés), titre exécutoire contestable (si le jugement est frappé d'appel, la saisie peut être suspendue), ou situation financière changée (si le salarié change d'emploi ou voit ses revenus chuter).
...
Si le salarié change d'emploi pendant la saisie, le nouveau employeur doit être notifié par l'huissier. La saisie continue automatiquement avec le nouvel employeur. Si le salarié离职 ne reprend pas d'emploi salarié, d'autres mesures d'exécution peuvent être envisagées (saisie sur compte bancaire, saisie de biens).
...
Les dernières valeurs de l'IRL publiées (pour référence) : IRL Q3 2025 (publié octobre 2025) : environ 146,2. IRL Q4 2025 (publié janvier 2026) : environ 146,5. IRL Q1 2026 (publié avril 2026) : disponible sur insee.fr.
...
Inconvénients : mise à jour manuelle de chaque paiement, risque d'erreur de calcul, pas d'alertes automatiques pour les échéances, sécurité faible (fichiers locaux ou Google vite hackés), et impossible à vérifier la conformité juridique des modèles.

#### Les applications gratuites de suivi locatif

Plusieurs applications proposent un suivi basique des paiements sans frais. Applications gratuites notables : Google Calendar (rappel des échéances de paiement, gratuit mais basique), Notion (modèles de suivi locatif, gratuit pour usage personnel), Sheetgo (automatisation des tableaux, gratuit jusqu'à certain nombre de connexions).

Limitations : fonctionnalités réduites, pas de génération de quittances certifiées, pas de modèles de bail évolutifs, pas de mise à jour juridique automatique, et support client limité ou inexistant.
...
Limitations : fonctionnalités réduites en gratuit (nombre de biens limité, pas d'accès aux modèles premium), absence de support en version gratuite, et pas de mise à jour juridique automatique.

### Ce qui n'est jamais vraiment gratuit

Certaines offres dites gratuites ne sont en réalité pas gratuites. Les agences immobilières qui proposent la gestion gratuite répercutent leurs frais sur le locataire (frais de dossier élevés) ou sur le propriétaire (commission cachée dans le taux de gestion). Les modèles de bail gratuits sur internet sont souvent obsolètes et non conformes aux dernières évolutions législatives. Les générateurs de quittances gratuit en ligne génèrent des documents non conformes ou watermarées, impropres à un usage professionnel.

### Les vrais coûts de la gestion locative gratuite

La gestion locative gratuite présente des coûts cachés. Temps passé : un propriétaire qui gère lui-même ses locations sur Excel ou Google Sheets passe en moyenne 2 à 4 heures par mois et par bien à la gestion administrative. Erreurs et litiges : un bail mal rédiger ou une quittance non conforme peut entraîner des litiges avec le locataire. Risque juridique : l'absence de mise à jour des modèles de bail peut entraîner des clauses nulles. Manque à gagner sur les paiements : un impayé non détecté à temps peut coûter des mois de loyer non recovered.

### Quand la gestion gratuite devient payante

La gestion locative gratuite devient réellement payante dans deux cas. À partir de 3 biens : le temps de gestion augmente proportionnellement. À partir de 3 biens, le coût d'un logiciel de gestion (généralement 10 à 30 euros par mois) est compensé par le temps économisé. En cas de litige : un seul litige avec un locataire (impayé, dégradation, procédure d'expulsion) peut coûter entre 500 et 5 000 euros en frais de procédure.

### Les alternatives abordables à la gestion gratuite

Si le budget est limité, voici les alternatives les plus pertinentes. Le régime auto-entrepreneur : en déclarant vos revenus locatifs en auto-entrepreneur, vous pouvez bénéficier d'un taux de CFE réduit et déduire certaines charges. Les partenariats avec des property managers : certains property managers proposent des tarifs réduits pour les propriétaires qui gèrent plusieurs biens. La garantie Visale : cette garantie gratuite de l'État protège contre les impayés. Elle remplace efficacement une GLI payante pour les locataires de moins de 30 ans ou en CDD.

### Nos recommandations

La gestion locative gratuite est viable pour les propriétaires avec un seul bien et un calme locatif parfait (loyers reçus à temps, locataire stable, pas de travaux). Au-delà, les risques et le temps investi dépassent largement le coût d'un outil spécialisé.
...
**Fourchette de prix habituels :** Gestion complète : 5% à 10% du montant des loyers annuels. Gestion technique seule : 3% à 5% (sans mission de relocation). Location seule (mise en location + bail) : 1 mois de loyer environ.
...
**Prestations incluses dans la gestion complète :** rédaction et signature du bail, état des lieux d'entrée et de sortie, encaissement des loyers et suivi des paiements, relances en cas d'impayé (lettres, appels), régularisation des charges annuelles, déclarations fiscales (revenus fonciers), suivi des travaux et des maintenance, et correspondance avec le locataire.

**Prestations généralement facturées en extra :** location de votre bien (placement d'un nouveau locataire) : 1 mois de loyer ou forfait. Procédure d'expulsion : frais d'huissier et d'avocat à la charge du propriétaire. Travaux de rénovation ou d'aménagement : frais de coordination. Assurance GLI : parfois proposée en option (souvent coûteuse).
...
| Solution | Coût annuel (1 bien) | Coût annuel (5 biens) |
|---|---|---|
| Agence (gestion complète) | 600-1 200 € | 3 000-6 000 € |
| Logiciel milieu de gamme | 120-360 € | 600-1 800 € |
| Auto-gestion (Excel) | 0 € | 0 € |
...
De nouveaux acteurs proposent des services de gestion locative à prix réduit, en digitalisant les processus et en limitant les frais de structure. Les property techs : entreprises comme Loiselet & Daugabel, Exitim, ou Optim-home proposent des gestions à partir de 4-5% des loyers, avec un service partiellement digitalisé. Les marketplaces de gestion : des plateformes en ligne mettent en relation propriétaires et gestionnaires professionnels, avec des tarifs négociés. Les coffre-forts numériques : services comme Helios, Digikeo ou Locative permettent de numériser et stocker tous les documents locatifs, facilitant l'auto-gestion.

### Comment choisir son prestataire de gestion locative ?

Critères de choix : transparence des tarifs (méfiez-vous des offres dont le prix total n'est pas clair dès le départ), mise à jour juridique (le prestataire doit garantir la conformité de ses modèles de bail et de quittances avec les dernières évolutions légales), service client (réactivité en cas de problème), références et avis (consultez les avis en ligne et les retours d'expérience), et essai gratuit (un bon prestataire doit proposer un essai gratuit de 14 à 30 jours).
...
Oui, les frais de gestion (honoraires d'agence, frais de logiciel) sont déductibles des revenus fonciers dans le cadre du régime réel. Vous pouvez les ajouter à vos autres charges (travaux, assurances, intérêts d'emprunt) pour réduire votre revenu foncier imposable.

**Peut-on négocier les frais de gestion avec une agence ?**

Oui, notamment si vous avez plusieurs biens ou si votre bien est en zone tendue (les agences sont moins sélectives). N'hésitez pas à demander un devis détaillé et à négocier les honoraires.`,
  },
];
...
newArticles.forEach(art => {
  if (!existingSlugs.has(art.slug)) {
    console.log('Adding:', art.slug, '| content length:', art.content.length);
    added++;
  } else {
    console.log('SKIP:', art.slug);
  }
});
console.log('Total to add:', added);
console.log('Final total will be:', 44 + added);