...
Travaux à la charge du propriétaire : gros murs et structure du bâtiment, ravalement et façades, mise aux normes d'accessibilité des parties communes, et installations collectives.
...
Si le bailleur ne répond pas à la lettre recommandée dans 15 jours, saisisissez le juge des contentieux de la protection. Le délai de 2 mois pour la restitution du dépôt continue de courir, et le retard peut générer des intérêts au taux légal.
...
Une lettre de relance doit contenir les informations suivantes : vos coordonnées complètes (nom, adresse, téléphone, email), les coordonnées du locataire (nom, prénom, adresse du logement), la référence du bail (date du bail, adresse du bien), le montant de l'impayé (montant exact des arriérés avec le décompte), la période concernée, le délai imparti pour le paiement (généralement 8 à 15 jours), et les conséquences en cas de non-paiement (mise en demeure, procédure judiciaire).
...
Le modèle doit être envoyé en recommandé avec accusé de réception. Il doit mentionner l'identité des parties, la référence du bail, le montant exact de l'impayé avec décompte, la période concernée, le délai de paiement (8 jours recommandé), et les conséquences juridiques du non-paiement (procédure de recouvrement, résiliation du bail).

Après la lettre de relance : la mise en demeure

Si le locataire ne réagit pas dans le délai imparti, la mise en demeure est l'étape obligatoire suivante. Elle doit être envoyée en recommandé avec accusé de réception et reprendre les mêmes éléments, avec une formulation plus ferme. La mise en demeure marque le passage à la phase contentieuse. Elle doit explicitement évoquer les conséquences juridiques du non-paiement (résiliation du bail, procédures d'expulsion).
...
La chronologie recommandée : Jour 1 (constatation de l'impayé), Jour 5-10 (lettre de relance en recommandé), Jour 15-20 (mise en demeure si pas de réponse), Jour 30+ (assignation au tribunal si toujours pas de réponse).

### Les effets juridiques de la mise en demeure

La mise en demeure produit plusieurs effets. Elle interrompt la prescription : à compter de la mise en demeure, le délai de prescription des actions en paiement de loyer est interrompu. Elle constitue une preuve formelle que le propriétaire a alerté le locataire. Elle ouvre le droit aux intérêts de retard (au taux légal). Elle permet la résiliation du bail si le locataire ne paie pas dans le délai.
...
Barème de saisie sur salaire (en pourcentage du salaire net) : jusqu'à 559,17 euros (0%), de 559,17 à 895,47 euros (10%), de 895,47 à 1 193,96 euros (25%), de 1 193,96 à 1 492,45 euros (35%), de 1 492,45 à 1 790,94 euros (45%), de 1 790,94 à 2 089,43 euros (55%), de 2 089,43 à 2 387,93 euros (65%), de 2 387,93 à 2 686,42 euros (75%), au-delà de 2 686,42 euros (85%).
...
La formule de révision du loyer est la suivante : Nouveau loyer = Loyer actuel × (Nouvel IRL / Ancien IRL)

Exemple : un appartement loué 800 euros en janvier 2025, révision en janvier 2026 : Loyer actuel 800 euros, IRL au 1er janvier 2025 : 145,0, IRL au 1er janvier 2026 : 146,5, Calcul : 800 × (146,5 / 145,0) = 808,28 euros. L'augmentation est de 8,28 euros par mois (environ 1%).
...
Les tableurs Excel et Google Sheets constituent la solution la plus basique. Cette méthode est gratuite et simple à mettre en place pour les propriétaires avec 1 ou 2 biens. Avantages : coût zéro, flexibilité totale, pas de dépendance à un service tiers. Inconvénients : mise à jour manuelle de chaque paiement, risque d'erreur de calcul, pas d'alertes automatiques pour les échéances, sécurité faible, et impossible de vérifier la conformité juridique des modèles.

Plusieurs applications proposent un suivi basique des paiements sans frais. Applications gratuites notables : Google Calendar (rappel des échéances de paiement), Notion (modèles de suivi locatif), Sheetgo (automatisation des tableaux). Limitations : fonctionnalités réduites, pas de génération de quittances certifiées, pas de modèles de bail évolutifs, pas de mise à jour juridique automatique.

Certains logiciels de gestion locative proposent une version gratuite (freemium) avec des fonctionnalités limitées. Cette version gratuite est souvent une vitrine pour la version payante. Limitations : fonctionnalités réduites en gratuit, absence de support en version gratuite, pas de mise à jour juridique automatique.
...
Fourchette de prix habituels : Gestion complète (5% à 10% du montant des loyers annuels), Gestion technique seule (3% à 5%), Location seule (1 mois de loyer environ).
...
Fourchette de prix habituels : Entrée de gamme (gratuit à 10 euros/mois) pour suivi basique des paiements, modèles de bail simples, convient aux propriétaires avec 1-2 biens. Milieu de gamme (10 à 30 euros/mois) pour gestion complète des baux, quittances automatisées, comptabilité, alertes, convient à 5-10 biens. Haut de gamme (30 à 100 euros/mois) pour gestion de portefeuille complète, intégration comptable, portail locataire, convient aux property managers professionnels.

Comparaison rapide : Agence (gestion complète) 600-1 200 euros/an pour 1 bien, 3 000-6 000 euros pour 5 biens. Logiciel milieu de gamme : 120-360 euros/an pour 1 bien, 600-1 800 euros pour 5 biens. Auto-gestion (Excel) : 0 euro pour 1 ou 5 biens.
...
# Helper to format a single article entry for TypeScript
def fmt_article(art):
    # Escape backticks and ${} inside the template literal content
    esc = art['content'].replace('`', '\\`').replace('${', '\\${')
    return f"""  {{
    slug: "{art['slug']}",
...
# Find the last article entry before the closing bracket
...
print(f"Last entry end position: {last_entry_end}")
...
for art in new_articles:
    if art['slug'] not in existing_slugs:
        new_entries.append(fmt_article(art))
        print(f"Adding: {art['slug']} ({len(art['content'])} chars)")
    else:
        print(f"SKIP (already exists): {art['slug']}")

new_content = '\n'.join(new_entries) + '\n' + content[last_entry_end:]
new_final = content[:last_entry_end] + '\n'.join(new_entries) + '\n' + content[last_entry_end:]

with open('src/data/articles.ts', 'w') as f:
    f.write(new_final)

print(f"\nDone. Added {len(new_entries)} articles.")
